...
void getClicks(Window w1, Window w2,
    ...
    while (button != 3)
    {
        if (clickedWindow == w1)
        {
            ...
            drawPoint(clickedPoint , GREEN);
            drawString(clickedPoint , to_string(pointNumber) , GREEN);
            cout << "Recorded point n. " << pointNumber << " situated at coordinates x,y = " << clickedPoint << "on the first picture." << endl;
        }
        else if (clickedWindow == w2)
        {
            setActiveWindow(w2);
            ...
            drawPoint(clickedPoint , GREEN);
            drawString(clickedPoint , to_string(pointNumber) , GREEN);
            cout << "Recorded point n. " << pointNumber << " situated at coordinates x,y = " << clickedPoint << "on the second picture." << endl;
        }
        else
        {
    ...
    cout << "Right click detected" << endl << "NEXT STEP" << endl;
}
...
void panorama(const Image<Color,2>& I1, const Image<Color,2>& I2,
    ...
    for (int x = 0 ; x < I.width() ; x++)
    {
        for (int y = 0 ; y < I.height() ; y++)
        {
            ...
            xyz[1] = y+y0;
            xyz[2] = 1;
            if (xyz[0] >= 0 && xyz[0] < I2.width() && xyz[1] >= 0 && xyz[1] < I2.height())
            {
